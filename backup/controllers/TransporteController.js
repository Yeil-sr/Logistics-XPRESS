...
static async createTeste(req, res) {
  try {
    const dados = req.body;
    const transporte = await TransporteController
      .getTransporteServices()
      .createTransporteTeste(dados);
    
    return res.status(201).json({
      message: 'Transporte de teste criado com sucesso',
      transporte
    });
  } catch (error) {
    return res.status(400).json({ 
      error: 'Erro ao criar transporte de teste', 
