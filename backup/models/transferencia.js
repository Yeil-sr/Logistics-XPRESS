...
module.exports = (sequelize, DataTypes) => {
  ...
  Transferencias.init({
    numero_TO: { type: DataTypes.STRING, allowNull: true },
    ...
    tipo_recebedor: { type: DataTypes.ENUM('HUB', 'STATION'), allowNull: true },
    quantidade: { type: DataTypes.INTEGER, allowNull: true },
    peso_kg: { type: DataTypes.DECIMAL(12, 2), allowNull: true },
    direcao: { type: DataTypes.ENUM('OUTBOUND', 'INBOUND'), allowNull: true },
    operador_id: { type: DataTypes.INTEGER, allowNull: true },
    status: { type: DataTypes.ENUM('CRIADO', 'EM_TRANSPORTE', 'RECEBIDO', 'CANCELADO'), allowNull: false, defaultValue: 'CRIADO' },
