...
class MotoristasServices extends Services {
  ...
  async findMotorista(identifier) {
    try {
      ...
      if (!isNaN(id)) {
        return await db.Motoristas.findByPk(id, {
          include: [
            {
        ...
        return await db.Motoristas.findOne({
          where: {
            nome: {
              [Op.iLike]: `%${identifier}%`
            },
  ...
  async getAllMotoristasAtivos(options = {}) {
    ...
    try {
      const { count, rows } = await db.Motoristas.findAndCountAll({
  ...
  async getColetasByMotorista(id) {
    try {
      const motorista = await db.Motoristas.findByPk(id, {
        include: [
          {
            model: db.Coletas,
            as: 'coletas',
  ...
  async getEstatisticasMotorista(id) {
    try {
      ...
      const rotas = await db.Rotas.findAll({
  ...
  async createMotorista(data) {
    ...
    try {
      ...
      const motoristaExistente = await db.Motoristas.findOne({
        where: { cpf: data.cpf },
        transaction
      });
      ...
      if (data.cnh) {
        const cnhExistente = await db.Motoristas.findOne({
          where: { cnh: data.cnh },
          transaction
        });
  ...
  async updateMotorista(id, data) {
    ...
    try {
      ...
      if (data.cpf && data.cpf !== motorista.cpf) {
        const cpfExistente = await db.Motoristas.findOne({
          where: { cpf: data.cpf },
          transaction
        });
      ...
      await db.Motoristas.update(dadosAtualizacao, {
        where: { id: Number(id) },
        transaction
      });
  ...
  async verificarDisponibilidade(motoristaId) {
    try {
      const motorista = await db.Motoristas.findByPk(motoristaId);
  ...
  async restoreMotorista(id) {
    try {
      await db.Motoristas.restore({ where: { id: Number(id) } });
      return await this.getMotoristaById(id);
    } catch (error) {
